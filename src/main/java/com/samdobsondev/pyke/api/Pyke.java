...
import com.samdobsondev.pyke.api.detector.ActivePlayerEventDetector;
import com.samdobsondev.pyke.api.detector.AllPlayersEventDetector;
import com.samdobsondev.pyke.api.detector.AnnouncerNotificationEventDetector;
import com.samdobsondev.pyke.api.detector.GameDataEventDetector;
...
import com.samdobsondev.pyke.api.watcher.PortWatcher;
import com.samdobsondev.pyke.exception.BaselineResponseException;
import com.samdobsondev.pyke.exception.SSLContextCreationException;
import com.samdobsondev.pyke.exception.SSLContextCreationFailedException;
...
public class Pyke {
    ...
    private static final Logger logger = LoggerFactory.getLogger(Pyke.class);
        ...
        pollingTask = scheduler.scheduleAtFixedRate(() -> {
            try {
                ...
                if (!apiResponse.success()) {
                    ...
                    // Check if we're in the 'limbo' state
                    if (incomingResponse.getActivePlayer() == null || incomingResponse.getActivePlayer().getAbilities() == null || incomingResponse.getAllPlayers().isEmpty()) {
                        logger.info("Incomplete game data received, game is still loading...");
                    } else {
                        // Detects changes, generates events and dispatches events that have occurred to the relevant listeners
        ...
        for (ActivePlayerEvent activePlayerEvent : activePlayerEvents) {
            for (ActivePlayerEventListener listener : activePlayerEventListeners) {
                switch (activePlayerEvent.getActivePlayerEventType()) {
                    ...
                    case PASSIVE -> {
                        assert activePlayerEvent instanceof PassiveEvent;
                        listener.onPassive((PassiveEvent) activePlayerEvent);
                    }
