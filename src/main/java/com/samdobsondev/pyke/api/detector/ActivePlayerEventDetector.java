...
import com.samdobsondev.pyke.model.data.activeplayer.fullrunes.StatRune;
...
import com.samdobsondev.pyke.model.data.activeplayer.abilities.*;
...
public class ActivePlayerEventDetector {
    public List<ActivePlayerEvent> detectEvents(AllGameData currentAllGameData, AllGameData incomingAllGameData, Double eventTime) {
        ...
        // Check for passive
        if (!current.getAbilities().getPassive().equals(incoming.getAbilities().getPassive())) {
            PassiveEvent event = new PassiveEvent(ActivePlayerEventType.PASSIVE,
                    eventTime,
                    incomingAllGameData,
                    incoming.getAbilities().getPassive());
            events.add(event);
        }
